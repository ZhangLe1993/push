...
public final class StandardReadMappings {
    ...
    public static ReadMapping tinyintReadMapping() {
        return ReadMapping.longReadMapping(TINYINT, ResultSet::getByte);
    }

    public static ReadMapping smallintReadMapping() {
        return ReadMapping.longReadMapping(SMALLINT, ResultSet::getShort);
    }

    public static ReadMapping integerReadMapping() {
        return ReadMapping.longReadMapping(INTEGER, ResultSet::getInt);
    }

    public static ReadMapping bigintReadMapping() {
        return ReadMapping.longReadMapping(BIGINT, ResultSet::getLong);
    }

    public static ReadMapping realReadMapping() {
        return ReadMapping.longReadMapping(REAL, (resultSet, columnIndex) -> floatToRawIntBits(resultSet.getFloat(columnIndex)));
    }
    ...
    public static ReadMapping decimalReadMapping(DecimalType decimalType) {
        ...
        if (decimalType.isShort()) {
            return ReadMapping.longReadMapping(decimalType, (resultSet, columnIndex) -> encodeShortScaledValue(resultSet.getBigDecimal(columnIndex), scale));
        }
        return ReadMapping.sliceReadMapping(decimalType, (resultSet, columnIndex) -> {
            if (resultSet.getBigDecimal(columnIndex).scale() <= scale) {
                return encodeScaledValue(
                        resultSet.getBigDecimal(columnIndex),
                        scale);
            } else {
                return encodeScaledValue(
                        resultSet.getBigDecimal(columnIndex).setScale(scale, RoundingMode.HALF_UP),
                        scale);
            }
        });
    }
    ...
    public static ReadMapping charReadMapping(CharType charType) {
        ...
        return ReadMapping.sliceReadMapping(charType, (resultSet, columnIndex) -> Slices.utf8Slice(CharMatcher.is(' ').trimTrailingFrom(resultSet.getString(columnIndex))));
    }

    public static ReadMapping varcharReadMapping(VarcharType varcharType) {
        return ReadMapping.sliceReadMapping(varcharType, (resultSet, columnIndex) -> Slices.utf8Slice(resultSet.getString(columnIndex)));
    }

    public static ReadMapping varbinaryReadMapping() {
        return ReadMapping.sliceReadMapping(VARBINARY, (resultSet, columnIndex) -> Slices.wrappedBuffer(resultSet.getBytes(columnIndex)));
    }

    public static ReadMapping dateReadMapping() {
        return ReadMapping.longReadMapping(DATE, (resultSet, columnIndex) -> {
            /*
    ...
    public static ReadMapping timeReadMapping() {
        return ReadMapping.longReadMapping(TIME, (resultSet, columnIndex) -> {
            /*
    ...
    public static ReadMapping timestampReadMapping() {
        return ReadMapping.longReadMapping(TIMESTAMP, (resultSet, columnIndex) -> {
            /*
        ...
        switch (type.getJdbcType()) {
            ...
            case Types.DECIMAL:
                ...
                if (precision > Decimals.MAX_PRECISION) {
                    return Optional.of(decimalReadMapping(createDecimalType(38, 12)));//for postgres sql number类型,直接转换为mysql中的decimal(38,12)类型
                }
